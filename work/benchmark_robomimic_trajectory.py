...
def main():
    ...
    for b in test_b:
        ...
        rows.append({'demo':b.metadata['demo'],'ade_sync':ade(pred,b.future),'fde_sync':fde(pred,b.future),'ade_persistence':ade(np.repeat(b.observed[-1][None,:],len(b.future),axis=0),b.future),'fde_persistence':fde(np.repeat(b.observed[-1][None,:],len(b.future),axis=0),b.future),'fallback':fallback,'safety_violation':violation,'distance':dist})
    out=ROOT/'outputs'/'benchmark_runs'/'robot_manipulation'/'robomimic_lift_ph'; out.mkdir(parents=True,exist_ok=True)
    summary={'task_id':'robot_manipulation','dataset':'robomimic_lift_ph','status':'measured_alternative','n_train':len(train_b),'n_calibration':len(cal_b),'n_test':len(test_b),'sync_ade':bootstrap_ci([r['ade_sync'] for r in rows]),'sync_fde':bootstrap_ci([r['fde_sync'] for r in rows]),'persistence_ade':bootstrap_ci([r['ade_persistence'] for r in rows]),'persistence_fde':bootstrap_ci([r['fde_persistence'] for r in rows]),'fallback_rate':fallback_rate([r['fallback'] for r in rows]),'safety_violation_rate':safety_violation_rate([r['safety_violation'] for r in rows]),'gate_threshold':threshold,'safety_limit':safety_limit,'failure_notes':'nearest-context retrieval SYNC formulation; no closed-loop success claim'}
