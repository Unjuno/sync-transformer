...
def main():
    ...
    rows.append(dict(zip(FIELDS, ['robot_manipulation','RoboMimic lift/ph','10','demo 60/20/20','Vanilla/SYNC/Persistence','trajectory',f"{robot['sync_ade']['mean']:.6f}",'trajectory bootstrap CI','ADE',f"{robot['fallback_rate']:.6f}",'recorded',robot['failure_notes']+'; vanilla_transformer.json included','measured_alternative'])))
    ...
    rows.append(dict(zip(FIELDS, ['robot_trajectory','UCI pedestrian in traffic','12','track 60/20/20','Vanilla/SYNC/Persistence','trajectory',f"{traj['sync_ade']['mean']:.6f}",'trajectory bootstrap CI','ADE',f"{traj['fallback_rate']:.6f}",'recorded',traj['failure_notes']+'; vanilla_transformer.json included','measured_alternative'])))
    pending = []
