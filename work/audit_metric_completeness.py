...
def main():
    ...
    for p in sorted((ROOT/'outputs'/'benchmark_runs').glob('*/*/summary.json')):
        d=json.loads(p.read_text()); rows.append({'artifact':str(p.relative_to(ROOT)),'task':d.get('task_id'),'status':d.get('status'),'has_mae':('mae' in d or 'mean_sync_mae' in d),'has_latency':('latency_ms' in d or 'latency' in d),'has_uncertainty':any(k in d for k in ('paired_delta_95ci','sync_ade','sync_fde','tracking_error')),'has_fallback':any(k in d for k in ('fallback_rate','mean_gate_use_rate')),'notes':d.get('failure_notes','')})
