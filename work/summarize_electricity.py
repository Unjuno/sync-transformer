...
def main():
    ap = argparse.ArgumentParser(); ap.add_argument('--mt001', required=True); ap.add_argument('--mt002', required=True); ap.add_argument('--output', required=True)
    a, b = (json.loads(Path(p).read_text()) for p in (ap.parse_args().mt001, ap.parse_args().mt002))
    rows = []
    for client, d in [('MT_001', a), ('MT_002', b)]:
