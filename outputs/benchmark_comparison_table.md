...
|---|---:|---:|---:|---|
| ETTh1 | 0.0691 | 0.202044 | 0.290602 | conditional; endpoint+seasonal delta over learned head -0.033155 |
| ETTh2 | 0.1590 | 0.573496 | 0.536355 | abstains in the canonical condition |
| ETTm1 | 0.0698 | 0.234096 | 0.463297 | conditional; endpoint+seasonal delta over learned head -0.009465 |
| ETTm2 | 0.1588 | 0.407707 | 0.481887 | conditional; endpoint+seasonal delta over learned head +0.013750 |

The standard Vanilla Transformer has 70,680 parameters on hourly datasets and 75,360 on minute datasets. The small model uses 10,424 and 12,800 parameters respectively; it is a capacity control, not a tuned state-of-the-art implementation. The SYNC deltas are from the canonical SYNC protocol and are reported as deltas rather than being mixed with the Vanilla absolute-MSE protocol. A fully matched-epoch, matched-parameter comparison remains a follow-up experiment.
...
Capacity-control artifacts are `outputs/vanilla_small_ETTh1.json`, `outputs/vanilla_small_ETTh2.json`, `outputs/vanilla_small_ETTm1.json`, `outputs/vanilla_small_ETTm2.json`, and `outputs/vanilla_transformer_small_benchmark.json`.
