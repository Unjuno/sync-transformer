...
SYNC is intended for recurring or seasonal processes where comparable past episodes exist and a fallback is available. Plausible application families include electricity load, renewable power, traffic, HVAC, server workload, retail demand, industrial sensors, and repetitive robot trajectories. The public ETT suite, three UCI electricity client series, and three BDG2 HVAC meters are measured; the remaining applications are hypotheses and are listed in [the task map](docs/TASKS.md). Electricity and HVAC results are series-dependent and do not establish general advantage (see the [cross-client summary](outputs/electricity_cross_client_summary.json)).
